...
const ITEMS = [
  {
    "label": "Acknowledge",
    "desc": "Employee confirms they've read the record",
    "icon": "✅"
  },
  {
    "label": "Appeal",
    "desc": "Employee can formally appeal the decision",
    "icon": "📤"
  },
  {
    "label": "Expunge",
    "desc": "Remove record after review period",
    "icon": "🗑️"
  },
  {
    "label": "Audit Trail",
    "desc": "Every action is logged permanently",
    "icon": "📝"
  }
];

export const DPAcknowledgeExpungeScene: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ background: COLORS.bg, fontFamily, padding: "40px 60px" }}>
      ...
      <Interactive.Div name="Breadcrumb" style={{ fontSize: 14, fontFamily: monoFamily, color: COLORS.textMuted, marginBottom: 10, opacity: interpolate(frame, [0, 0.3 * fps], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }}>
        <span style={{ color: COLORS.accentBright }}>People & HR</span><span style={{ margin: "0 8px" }}>{"›"}</span>
        <span style={{ color: COLORS.accentBright }}>Compliance</span><span style={{ margin: "0 8px" }}>{"›"}</span>
        <span style={{ color: COLORS.accentBright }}>Disciplinary</span><span style={{ margin: "0 8px" }}>{"›"}</span>
        <span style={{ color: COLORS.white }}>Actions</span>
      </Interactive.Div>
