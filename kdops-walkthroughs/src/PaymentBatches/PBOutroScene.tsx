...
export const PBOutroScene: React.FC = () => {
  ...
  return (
    <AbsoluteFill
      style={{
        ...
        display: "flex", flexDirection: "column" as const, justifyContent: "center", alignItems: "center", fontFamily,
      }}
    >
      ...
      <Interactive.Div name="CheckIcon" style={{ fontSize: 64, marginBottom: 24, scale: interpolate(frame, [0.3 * fps, 0.8 * fps], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp", easing: Easing.spring({ damping: 200 }) }) }}>
        💳
      </Interactive.Div>
      ...
      <Interactive.Div name="Subtitle" style={{ fontSize: 22, color: COLORS.textMuted, textAlign: "center" as const, marginTop: 16, maxWidth: 600, lineHeight: 1.6, opacity: interpolate(frame, [1 * fps, 1.5 * fps], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }}>
        You can now create batches, manage approvals, and track payments across your organisation.
      </Interactive.Div>
      ...
      <Interactive.Div name="UpNext" style={{ display: "flex", gap: 16, marginTop: 48, opacity: interpolate(frame, [1.5 * fps, 2 * fps], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }}>
        {["Payroll Intelligence", "Document Vault", "Compliance Tracker"].map((title) => (
          <div key={title} style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 10, padding: "16px 24px", textAlign: "center" as const }}>
            <div style={{ fontSize: 12, color: COLORS.accentBright, fontFamily: monoFamily, marginBottom: 6 }}>UP NEXT</div>
