  ...
  return (
    <kbd
      className="inline-flex items-center justify-center min-w-[24px] h-[22px] px-1.5 rounded
        bg-[#F4F4F5] dark:bg-[hsl(220,25%,18%)]
        border border-[#E5E5E5] dark:border-[hsl(220,25%,25%)]
        text-2xs font-mono font-medium
        text-[#6A7184] dark:text-[hsl(220,25%,70%)]
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-5 pt-1">
          {SECTIONS.map((section) => (
            <div key={section.title}>
              <h3 className="text-2xs font-semibold uppercase tracking-wider text-[#6A7184] dark:text-[hsl(220,25%,60%)] mb-2 px-1">
                {section.title}
              </h3>
                ...
                {section.shortcuts.map((shortcut) => (
                  <div
                    ...
                    className="flex items-center justify-between py-1.5 px-2 rounded
                      hover:bg-[#F9F9FA] dark:hover:bg-[hsl(220,25%,14%)]"
                  >
                    ...
                    <div className="flex items-center gap-1 ml-4 shrink-0">
                      {shortcut.keys.map((key, i) => (
                        <span key={i} className="flex items-center gap-1">
                          {i > 0 && (
                            <span className="text-3xs text-[#9AA2AF] dark:text-[hsl(220,25%,45%)]">+</span>
                          )}
                          <Kbd>{key}</Kbd>
                        </span>
                      ))}
        ...
        <div className="pt-3 border-t border-[#E5E5E5] dark:border-[hsl(220,25%,18%)]">
          <p className="text-2xs text-[#9AA2AF] dark:text-[hsl(220,25%,50%)] text-center">
