...
export default function Grievances() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: gData }, { data: pData }] = await Promise.all([
      supabase.from('grievances').select('id, subject, description, category, severity, is_anonymous, reporter_id, assigned_to, status, resolution_notes, resolved_at, created_at').order('created_at', { ascending: false }),
