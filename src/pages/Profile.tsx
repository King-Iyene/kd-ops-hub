...
const ProfilePage = () => {
  ...
  const previewPayslip = async (p: Payslip) => {
    ...
    try {
      ...
      // Rendered in-page rather than window.open()'d — a real user hit a
      // browser/site pop-up permission that blocked it outright, which no
      // amount of gesture-timing fixes can get around.
      if (p.storage_path) {
        ...
        if (data?.signedUrl) {
          ...
          if (res.ok) {
            ...
            setPayslipPreview({ title: `Payslip — ${p.period}`, html, filename: `payslip-${p.period}` });
            return;
          }
        }
      }
