                    ...
                    {editingWorkflow.steps.map((step, idx) => (
                      <div key={step.id} className="flex items-center gap-2 rounded-md border p-2">
                        ...
                        <div className="flex gap-0.5">
                          <Button variant="ghost" size="icon-sm" aria-label="Move up" disabled={idx === 0} onClick={() => moveStep(idx, 'up')}>
                            <ArrowUp className="h-3.5 w-3.5" />
                          </Button>
                          <Button variant="ghost" size="icon-sm" aria-label="Move down" disabled={idx === editingWorkflow.steps.length - 1} onClick={() => moveStep(idx, 'down')}>
                            <ArrowDown className="h-3.5 w-3.5" />
                          </Button>
                          <Button variant="ghost" size="icon-sm" className="text-destructive" aria-label="Remove step" onClick={() => removeStep(step.id)}>
