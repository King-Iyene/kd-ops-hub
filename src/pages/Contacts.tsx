              ...
              <Table>
                ...
                <TableBody>
                  {pagination.slice.map((c) => (
                    <TableRow key={c.id} className="kd-transition cursor-pointer hover:bg-muted/50 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-inset" tabIndex={0} role="link" onClick={() => navigate(`/contacts/${c.id}`)} onKeyDown={(ev) => { if (ev.key === 'Enter' || ev.key === ' ') { ev.preventDefault(); navigate(`/contacts/${c.id}`); } }} onAuxClick={(e) => { if (e.button === 1) { window.open(`/contacts/${c.id}`, '_blank'); e.preventDefault(); } }}>
