              ...
              <div className="h-[220px]">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={trendData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
                    ...
                    <YAxis tick={axisTick} axisLine={false} tickLine={false} domain={['auto', 'auto']} tickFormatter={(v: number) => `₦${v.toFixed(0)}`} />
                    <ReTooltip content={<GlassTooltip formatter={(v: number) => formatFxRate(v)} />} />
                    <Area type="stepAfter" dataKey="rate" name="USD/NGN rate" stroke={SERIES[0]} strokeWidth={2} fill="url(#kd-grad-primary)" dot={false} {...chartAnim} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </>
              ...
              <div className="h-[220px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={sensitivityData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }} barSize={20}>
                    ...
                    <ReTooltip content={<GlassTooltip formatter={(v: number) => formatNairaCompact(v)} />} cursor={{ fill: 'currentColor', fillOpacity: 0.04 }} />
                    <Bar dataKey="monthly_ngn" name="Monthly cost" radius={[6, 6, 0, 0]} {...chartAnim}>
