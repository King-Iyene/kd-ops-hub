                ...
                <CardHeader className="pb-2">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                      <Button size="icon-sm" variant="ghost" aria-label={note.pinned ? 'Unpin note' : 'Pin note'} onClick={() => togglePin(note)}>
                        {note.pinned ? <PinOff className="h-3.5 w-3.5" /> : <Pin className="h-3.5 w-3.5" />}
                      </Button>
                      <Button size="icon-sm" variant="ghost" aria-label="Edit note" onClick={() => openEdit(note)}>
                        <Pencil className="h-3.5 w-3.5" />
                      </Button>
                      <Button size="icon-sm" variant="ghost" aria-label="Delete note" onClick={() => handleDelete(note)}>
