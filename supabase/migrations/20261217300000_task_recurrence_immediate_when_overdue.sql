-- Completing a recurring task normally stashes the next occurrence's due
-- date and waits for the nightly sweep (midnight WAT) to actually create
-- it — right for the common case, where the next due date is still in
-- the future. But if the task being completed was already overdue, the
-- next cadence date can already be today or in the past by the time it's
-- closed — waiting for tonight's sweep for something that's already due
-- makes no sense. Create it immediately in that case instead.
