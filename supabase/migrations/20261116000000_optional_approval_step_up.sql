-- =============================================================================
-- Restore step-up (password + TOTP) re-authentication for payment/expense
-- approvals — but OPT-IN this time, not forced.
--
-- Background: 20260816000000 built a full step-up system (password + TOTP
-- required before every approve/reject). 20260817000000 ("remove
-- restrictions") deleted it entirely, on both backend and frontend, with no
-- stated reason — leaving zero server-side AAL2 enforcement on money
-- approval today. A second-pass forensic audit flagged this as Medium
-- ("MFA is decorative").
--
-- Restoring the ORIGINAL always-on version blindly would be actively
-- harmful right now: 0 of 10 current approvers (admin/operations/
-- super_admin) have TOTP enrolled, so a hard requirement would lock out
-- every approver from approving anything the moment this migration lands.
--
-- Fix: rebuild the full mechanism, but gate enforcement behind a company
-- setting (`approval_step_up_required`, default false — off, matching
-- today's behaviour exactly). A super_admin can turn it on once approvers
-- have enrolled 2FA, from Settings → Security.
-- =============================================================================


-- =============================================================================
-- Part 1 — company_settings flag
-- =============================================================================

ALTER TABLE public.company_settings
  ADD COLUMN IF NOT EXISTS approval_step_up_required boolean NOT NULL DEFAULT false;


-- =============================================================================
-- Part 2 — step-up tables (recreated; dropped by 20260817000000)
-- =============================================================================
...
DROP POLICY IF EXISTS su_self_read ON public.step_up_sessions;
...
-- =============================================================================
-- Part 3 — create_step_up_session / consume_step_up_token
--
-- Unchanged from the original design: password + verified TOTP required to
-- mint a token. These RPCs are always available; whether the approval RPCs
-- actually demand a token is controlled by the company_settings flag below.
-- =============================================================================
...
-- =============================================================================
-- Part 4 — Approval RPCs: optional p_step_up_token, enforced only when
-- company_settings.approval_step_up_required = true.
--
-- Signature changes from (uuid, text) to (uuid, uuid DEFAULT NULL, text
-- DEFAULT NULL) — explicit DROP of the old signature first since this is a
-- different overload, not a like-for-like CREATE OR REPLACE.
-- =============================================================================
...
CREATE OR REPLACE FUNCTION public.approve_payment_batch(
  p_batch_id        uuid,
  p_step_up_token   uuid DEFAULT NULL,
...
BEGIN
  ...
  IF COALESCE(v_step_up_on, false) THEN
    IF p_step_up_token IS NULL
       OR NOT public.consume_step_up_token(p_step_up_token, 'reject_batch', p_batch_id) THEN
      RAISE EXCEPTION 'step_up_required: Step-up token invalid, expired, or already consumed'
        USING ERRCODE = 'P0003';
    END IF;
  END IF;
...
REVOKE EXECUTE ON FUNCTION public.reject_payment_batch(uuid, text, uuid) FROM PUBLIC;
GRANT  EXECUTE ON FUNCTION public.reject_payment_batch(uuid, text, uuid)
  TO authenticated, service_role;
...
BEGIN
  ...
  IF COALESCE(v_step_up_on, false) THEN
    IF p_step_up_token IS NULL
       OR NOT public.consume_step_up_token(p_step_up_token, 'reject_expense', p_expense_id) THEN
      RAISE EXCEPTION 'step_up_required: Step-up token invalid, expired, or already consumed'
        USING ERRCODE = 'P0003';
    END IF;
  END IF;
...
REVOKE EXECUTE ON FUNCTION public.reject_expense(uuid, text, uuid) FROM PUBLIC;
GRANT  EXECUTE ON FUNCTION public.reject_expense(uuid, text, uuid)
  TO authenticated, service_role;

-- =============================================================================
-- Part 5 — enrollment-status helper for the Settings toggle UI, so a
-- super_admin can see "X of Y approvers have 2FA enrolled" before flipping
-- approval_step_up_required on, instead of finding out the hard way.
-- =============================================================================

CREATE OR REPLACE FUNCTION public.approver_totp_enrollment_status()
RETURNS TABLE(total_approvers int, enrolled_approvers int)
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
AS $$
    ...
    count(*) FILTER (
      WHERE EXISTS (
        SELECT 1 FROM auth.mfa_factors mf
         WHERE mf.user_id = p.id AND mf.factor_type = 'totp' AND mf.status = 'verified'
      )
    )::int AS enrolled_approvers
  FROM public.profiles p
  WHERE p.role IN ('super_admin', 'admin', 'operations')
    AND COALESCE(p.status, 'active') = 'active';
$$;

REVOKE EXECUTE ON FUNCTION public.approver_totp_enrollment_status() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.approver_totp_enrollment_status() TO authenticated;

-- End of migration.
-- Step-up mechanism restored and available; enforcement stays OFF
-- (company_settings.approval_step_up_required = false) until a super_admin
-- opts in from Settings → Security once approvers have enrolled TOTP.
