-- =============================================================================
-- Migration: 20261001000500_admin_can_approve_any_batch.sql
-- =============================================================================
-- Policy change: admin can approve ANY payment batch (or expense), including
-- ones created by super_admin, so that admins can act as the emergency
-- approver when a super_admin is unavailable. Finance and Operations remain
-- OUT of the approver pool by design.
--
-- What changed vs 20261001000200:
--   • The pool-narrowing block that forced `super_admin` only when the batch
--     creator is admin/super_admin is REMOVED. The default pool
--     ["admin","super_admin"] applies unconditionally.
--
-- What did NOT change:
--   • Self-approval policy: admin/super_admin can self-approve, others still
--     blocked at the RPC layer.
--   • approver_pools table content: still ["admin","super_admin"] for both
--     tiers on both payment_batch and expense_payment. Finance/operations
--     stay excluded.
--   • Every other RPC, trigger, or CHECK constraint.
--
-- Impact matrix (payment batches):
--   Batch created by → | Approver: super_admin | admin       | finance | ops
--   super_admin        |  ✅                    | ✅ (NEW)     | ❌       | ❌
--   admin              |  ✅                    | ✅ (NEW)     | ❌       | ❌
--   finance            |  ✅                    | ✅           | ❌       | ❌
--   operations         |  ✅                    | ✅           | ❌       | ❌
...
BEGIN
  ...
  ELSE
    -- Different caller: pool eligibility check. No narrowing — admin can
    -- approve any batch including one created by admin/super_admin, so
    -- admins can act as emergency approvers when super_admin is away.
...
BEGIN
  ...
  IF v_expense.submitted_by = v_caller THEN
    ...
    END IF;
  ELSE
    -- No narrowing — admin can approve any expense including one submitted
    -- by admin/super_admin. Emergency approver capability.
    SELECT eligible_roles INTO v_eligible_roles
      FROM public.approver_pools
